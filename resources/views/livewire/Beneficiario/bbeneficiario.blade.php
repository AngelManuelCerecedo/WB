<div class="w-full">
    <div class="bg-white border border-gray-200 rounded p-4">
        <div class=" flex sm:flex-row flex-col">
            ...
            <div class="sm:ml-8 mt-1">
                <a href="{{ route('RBeneficiarios') }}">
                    <button class="botoNUEVOSR">
                        <i class="bi bi-plus-lg text-lg"></i>
                        <span class="ml-2">Nuevo Beneficiario</span>
                    </button>
                </a>
            </div>
            <div class="sm:ml-52 mt-2">
                <label><i class="bi bi-house-door-fill"></i> <i class="bi bi-chevron-right"></i> Catálogos <i
                        class="bi bi-chevron-right"></i> Beneficiarios</label>
            </div>
        </div>
                        ...
                        @foreach ($beneficiarios as $beneficiario)
                            <tr>
                                <td data-label="ACCIONES :" class="lg:w-1/12">
                                    <a href="{{ route('EBeneficiarios', [$beneficiario->id]) }}">
                                ...
                                <td data-label="Nombre :">{{ $beneficiario->Nombre }}</td>
                                <td data-label="Banco :">{{ $beneficiario->Banco }}</td>
                                <td data-label="Cuenta :">{{ $beneficiario->Cuenta }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                {{ $beneficiarios->links() }} <!-- Mostrar enlaces de paginación -->
            @else
                <div class="px-6 py-4">
